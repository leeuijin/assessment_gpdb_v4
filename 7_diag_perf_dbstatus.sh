...
echo "####################" >> ${LOGFILE}
psql -c "select schemaname ,round(sum(pg_total_relation_size(schemaname||'.'||tablename))/1024/1024) as schema_size_MB from pg_tables WHERE schemaname NOT in('gp_toolkit','pg_catalog','public','information_schema','gpcc_schema','gpexpand')  group by 1;" >> ${LOGFILE}

#echo "" >> ${LOGFILE}
#echo "####################" >> ${LOGFILE}
#echo "### 4. SIZE per TABLE " >> ${LOGFILE}
#echo "####################" >> ${LOGFILE}
#psql -d gpperfmon -c "SELECT SCHEMA,table_name,relkind,relstorage,SIZE/1024/1024 AS size_mb FROM public.gpcc_size_ext_table
#WHERE SCHEMA NOT in('gp_toolkit','pg_catalog','public.,'dba','information_schema','gpcc_schema','gpexpand')
...
#echo "####################" >> ${LOGFILE}
#psql -d gpperfmon -c "SSELECT logmessage, count(*) FROM public.gpcc_pg_log_history GROUP BY 1 ORDER BY 2 DESC LIMIT 30;" >> ${LOGFILE}
...
echo "####################" >> ${LOGFILE}
psql -d gpperfmon -c "SELECT query_text,count(*) FROM public.queries_history WHERE ctime >= CURRENT_DATE - INTERVAL '7 days'
...
echo "####################" >> ${LOGFILE}
psql -d gpperfmon -c "SELECT db,username,query_text,avg (tfinish-tstart),max (tfinish-tstart) FROM public.queries_history
